    ...
    if (this.nesting) {
      ...
      Object.keys(groupedCheckPoints).forEach(k => {
        groupedCheckPoints[k] = groupedCheckPoints[k].map(e => {
          if (!e?.start || !e?.start?.time) throw new Error(`Start checkpoint ${e.start.event} doesn't exists`)
          if (!e?.end || !e?.end?.time) throw new Error(`End checkpoint ${e.start.event} doesn't exists`)
